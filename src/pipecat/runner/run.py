...
def _setup_telephony_routes(app: FastAPI, args: argparse.Namespace, ws_used_tokens: set[str]):
    ...
    if args.transport in TELEPHONY_TRANSPORTS:
        # XML response templates (Exotel and audiofork don't use XML webhooks)
        XML_TEMPLATES = {
...
<Response>
  <Stream bidirectional="true" keepCallAlive="true" contentType="audio/x-mulaw;rate=8000">wss://{args.proxy}/ws</Stream>
        ...
        async def start_call():
            ...
            if args.transport in ("exotel", "audiofork"):
                # Exotel and audiofork don't use POST webhooks - redirect to proper documentation
                logger.debug(f"POST {args.transport.upper()} endpoint - not used")
                return {
                    "error": f"{args.transport.upper()} doesn't use POST webhooks",
                    "websocket_url": f"wss://{args.proxy}/ws",
                    "note": f"Configure the WebSocket URL above in your {args.transport.upper()} App",
                }
            else:
...
def main(parser: argparse.ArgumentParser | None = None):
    ...
    Command-line arguments:
       ...
       - -t/--transport: Restrict to a single transport and set as default for /start
         (daily, webrtc, websocket, twilio, telnyx, plivo, exotel, audiofork). Omit to
